...
export const SQUADS={
  ...
  R:{name:'騎馬武者隊',count:8,unit:'騎',formation:'騎馬隊形',kind:'mounted',spacing:3.8,flightHeight:3.5,promoted:{name:'龍武者隊',formation:'昇龍隊形'}},
...
export const squadSpec=(type,promoted=false)=>promoted&&SQUADS[type]?.promoted?{...SQUADS[type],...SQUADS[type].promoted}:SQUADS[type];
export function formation(type, compact=false, promoted=false) {
  const spec=SQUADS[type];if(!spec)throw new Error('Unknown squad '+type);
  const out=[];
  if(spec.kind==='mounted') {
    for(let row=0;row<3;row++){
      const n=row===1?2:3;
      for(let col=0;col<n;col++)out.push({x:(col-(n-1)/2)*spec.spacing,z:(row-1)*3.1,altitudeOffset:promoted&&row===1?1.3:0,type:promoted?'D':type});
    }
