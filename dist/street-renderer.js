...
export function createStreetRenderer(scene,models,h){
 const batches=new Map(),root=new T.Object3D();let squads=[];
 for(const [type,model] of Object.entries(models)){
  model.updateMatrixWorld(true);const geometries=[];
  model.traverse(o=>{if(!o.isMesh)return;let g=o.geometry.clone().applyMatrix4(o.matrixWorld);if(g.index){const flat=g.toNonIndexed();g.dispose();g=flat;}g.scale(4,4,4);
   ...
   g.setAttribute('color',new T.BufferAttribute(colors,3));geometries.push(g);
  });
  const mesh=new T.InstancedMesh(mergeGeometries(geometries),new T.MeshStandardMaterial({vertexColors:true,roughness:.65,metalness:.18}),64);geometries.forEach(g=>g.dispose());mesh.count=0;mesh.frustumCulled=false;mesh.castShadow=true;mesh.receiveShadow=true;mesh.userData.cells=[];scene.add(mesh);batches.set(type,mesh);
 }
 function update(time){
  for(const b of batches.values()){b.count=0;b.visible=false;}
  for(const s of squads){
   const mesh=batches.get(s.piece.t),progress=s.defeat?.progress??0,fall=Math.max(0,Math.min(1,(progress-.5)/.25));
   ...
   mesh.setMatrixAt(mesh.count,root.matrix);mesh.userData.cells[mesh.count++]=s.ghost?-1:s.cell;mesh.visible=true;
  ...
  for(const b of batches.values())if(b.count)b.instanceMatrix.needsUpdate=true;return true;
 }
 function pickSurface(ray){const active=[...batches.values()].filter(b=>b.count);for(const b of active)b.computeBoundingSphere();const hit=ray.intersectObjects(active,false)[0];return hit?{point:hit.point,distance:hit.distance}:null;}
 return {setSquads:value=>squads=value,update,pickSurface,pickFlying:()=>null,contacts:()=>squads.flatMap(s=>(s.contacts??[]).map(p=>({...p,cell:s.cell}))),stats:()=>({soldiers:squads.length,representedSoldiers:squads.length,flyingRiders:0,detailedSquads:0,instanceBatches:[...batches.values()].filter(b=>b.count).length})};
}
