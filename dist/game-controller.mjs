...
export class GameController {
  ...
  async schedule(){
    ...
    try{
      ...
      await this.engine.init();if(!this.current(request))return;this.changed();
      ...
      if(choice.resign){this.activeRequest=null;this.match.resign(side);this.positionRevision++;this.invalidate();this.changed();return;}
      await this.commit(choice.m,'ai');
    }catch(error){
  ...
  async commit(m,actor){
    ...
    Promise.resolve().then(()=>{if(this.animations.has(event))return this.animate({...result,event});})
      ...
      .finally(()=>{if(this.animations.delete(event))this.changed();});
    this.schedule();
    return result;
  }
  undo(){
    ...
    this.changed();this.schedule();return true;
  }
  reset(){
