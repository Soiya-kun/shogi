...
test('promotion, reinforcement and check render kanji/subtitles without voices; undo/reset cancel',async({page})=>{
  ...
  await control(page,'#undo','click');expect(await page.locator('.war-banner').count()).toBe(0);expect(await page.evaluate(()=>window.__aether.state().g.b[31].p)).toBe(false);
  g=base();g.h[0].S=1;await load(page,g);await control(page,'#hands button','click');await cell(page,40);await logged(page,18);await expect(page.locator('.war-kanji')).toHaveText('援軍');
  ...
  await control(page,'#undo','click');expect(await page.locator('.war-banner').count()).toBe(0);
  await control(page,'#reset','click');await page.locator('#reset-confirm').click();expect(await page.evaluate(()=>window.__aether.war().log.length)).toBe(0);expect(errors).toEqual([]);
});
...
test('actual AI command acknowledgements use the current request, 3-second cadence and stop cancellation',async({page})=>{
  await load(page,new Match().g);await control(page,'#ai-toggle-1','click');expect(await page.evaluate(()=>window.__aether.war().log.some(e=>e.id===95))).toBe(false);
  await control(page,'#ai-toggle-0','click');await logged(page,95);
  ...
  await control(page,'#ai-stop-all','click');await logged(page,96);expect(await page.evaluate(()=>window.__aether.ai().settings.every(p=>!p.enabled))).toBe(true);
...
test('mobile reduced motion, subtitle/language switches and OFF retain check status and selection',async({page})=>{
  ...
  await control(page,'.war-settings summary','click');await control(page,'#war-mode','selectOption','off');await control(page,'#war-english','uncheck');await control(page,'#war-subtitles','uncheck');
...
test('manual tactical analysis uses real WASM in a separate worker and records a matching certificate',async({page})=>{
  ...
  expect(await page.locator('.war-banner').count()).toBe(0);await control(page,'#undo','click');expect(await page.evaluate(()=>window.__aether.state().presentation.proofs.length)).toBe(0);expect(errors).toEqual([]);
});
test('adding a local voice asset starts actual HTMLAudio and undo cancels playback',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>window.__aether.war().audio.playing)).toBe(true);await control(page,'#undo','click');expect(await page.evaluate(()=>window.__aether.war().audio.playing)).toBe(false);
});
