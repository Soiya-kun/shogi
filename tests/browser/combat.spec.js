...
test('piece labels follow ON/OFF for both battling squads and restored positions',async({page})=>{
  ...
  await labels(true);
  await control(page,'#labels','click');await labels(false);
  await control(page,'#labels','click');await labels(true);
  ...
  await control(page,'#labels','click');await control(page,'#undo','click');await labels(false);
...
test('every troop attacks, hits, staggers the enemy and occupies; promotion and rear army keep their identity',async({page})=>{
  ...
  for(const [type,promoted,side] of cases){
    ...
    await load(page,g);await cell(page,from);if(side)await camera(page,'rotate');await camera(page,'close');await page.waitForTimeout(850);
...
test('actual fast WASM AIs start further searches and independent captures before earlier battles finish',async({page})=>{
  ...
  await control(page,'#battle-sound','check');
  await page.evaluate(()=>{
  ...
  await control(page,'#ai-stop-all','click');
...
test('capture and immediate redeployment use distinct generations; undo, restore and effects OFF discard obsolete work',async({page})=>{
  ...
  await load(page,g);await control(page,'#battle-sound','check');
  ...
  await move(page,50,41,5);await move(page,40,49,6);await control(page,'#undo','click');await synced(page);await page.waitForTimeout(4500);await synced(page);
  ...
  await move(page,41,32,7);await control(page,'#battle-effects','uncheck');await synced(page);expect(await page.evaluate(()=>window.__aether.state().g.ply)).toBe(7);
  await control(page,'#battle-effects','check');await move(page,49,48,8);await control(page,'#reset','click');await page.locator('#reset-confirm').click();await page.waitForTimeout(4500);await synced(page);expect(await page.evaluate(()=>window.__aether.state().g.ply)).toBe(0);
});
...
test('rapid independent battles stay bounded, retain current-cell picking, and finish safely at game end',async({page})=>{
  ...
  await control(page,'#undo','click');await synced(page);expect(await page.evaluate(()=>window.__aether.state().g.ply)).toBe(0);
});
...
test('compact touch view bounds concurrent combat and reduced motion skips all delayed presentation',async({browser})=>{
  ...
  await control(page,'#ai-toggle-0','tap');await control(page,'#ai-toggle-1','tap');await page.waitForFunction(()=>window.__aether.state().g.ply>=8);await control(page,'#ai-stop-all','tap');
