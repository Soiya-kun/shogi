...
test('a static host without isolation headers prepares AI once and preserves the ongoing game',async({browser})=>{
  ...
  try{
    ...
    await manual(page);await settled(page);await control(page,'#ai-opening-1','selectOption','central');await control(page,'#ai-toggle-1','click');
    ...
    expect(await page.locator('#ai-opening-1').inputValue()).toBe('central');await control(page,'#ai-stop-all','click');
...
test('actual WASM: lazy load, rapid instructions, stop, both sides, restore and human decision undo',async({page})=>{
  ...
  await control(page,'#ai-opening-0','selectOption','fourth');await control(page,'#ai-toggle-0','click');
  ...
  await control(page,'#ai-toggle-0','click');await ply(page,1);await control(page,'#ai-toggle-0','click');
  await control(page,'#ai-opening-1','selectOption','central');await control(page,'#ai-toggle-1','click');await ply(page,2);await settled(page);
  ...
  await manual(page);await ply(page,4);await settled(page);await control(page,'#undo','click');await settled(page);
  expect(await page.evaluate(()=>window.__aether.state().g.ply)).toBe(2);await control(page,'#ai-stop-all','click');expect(errors).toEqual([]);
});
...
test('AI versus AI remains legal with live strategy changes, 3D rendering and pause on undo',async({page})=>{
  ...
  await control(page,'#ai-tempo','selectOption','fast');
  await control(page,'#ai-opening-0','selectOption','fourth');await control(page,'#ai-order-0','selectOption','attack');
  await control(page,'#ai-opening-1','selectOption','central');await control(page,'#ai-order-1','selectOption','counter');
  await control(page,'#ai-toggle-0','click');await control(page,'#ai-toggle-1','click');
  ...
  await control(page,'#ai-order-0','selectOption','defend');await control(page,'#ai-opening-1','selectOption','static');await ply(page,8);
  ...
  await control(page,'#ai-stop-all','click');await settled(page);const state=await page.evaluate(()=>window.__aether.state());
  ...
  await control(page,'#ai-toggle-0','click');await control(page,'#ai-toggle-1','click');await ply(page,state.g.ply+2);
  await expect(page.locator('#undo')).toBeEnabled({timeout:10000});await control(page,'#undo','click');await settled(page);
...
test('engine load failure leaves the board intact and retry recovers',async({page})=>{
  ...
  await page.goto('/?debug');await ready(page);await control(page,'#ai-toggle-0','click');await expect(page.locator('#ai-retry-0')).toBeVisible({timeout:25000});
  expect(await page.evaluate(()=>window.__aether.state().g.ply)).toBe(0);await page.unroute('**/ai/vendor/yaneuraou.wasm');
  await control(page,'#ai-retry-0','click');await ply(page,1);await control(page,'#ai-stop-all','click');await settled(page);
});
...
test('mobile commanders allow touch start and stop without horizontal overflow',async({browser})=>{
  ...
  await page.goto('http://127.0.0.1:5174/?debug');await ready(page);await control(page,'#ai-opening-0','selectOption','central');await control(page,'#ai-toggle-0','tap');await ply(page,1);
  await control(page,'#ai-stop-all','tap');await settled(page);expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
  await page.locator('#open-command').click();await page.locator('#ai-card-0').scrollIntoViewIfNeeded();await page.screenshot({path:'docs/verification/ai-mobile.png'});await context.close();
});
...
test('actual engine delivers a mating drop and a forced promoted move for the rear army',async({page})=>{
  ...
  for(const rear of [false,true]){
    ...
    await control(page,'#ai-toggle-'+(rear?1:0),'click');await expect.poll(()=>page.evaluate(()=>window.__aether.ai().phase),{timeout:20000}).toBe('ended');
