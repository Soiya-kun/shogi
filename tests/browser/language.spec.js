...
test('language switches UI and one dialogue line while preserving kanji titles, game state and saved preference',async({page})=>{
 ...
 await control(page,'#language','selectOption','en');await expect(page.locator('html')).toHaveAttribute('lang','en');
 ...
 await control(page,'#language','selectOption','ja');await expect(page.locator('.war-kanji')).toHaveText('昇龍');await expect(page.locator('.war-subtitle')).toHaveText('天を駆けよ！ 竜よ、戦場を制せ！');
 await expect(page.locator('#reset')).toHaveText('最初から');await expect(page.locator('#moveCount')).toHaveText('1 手');
 await control(page,'#language','selectOption','en');await control(page,'#reset','click');await expect(page.locator('#reset-title')).toHaveText('Start a new game?');await page.locator('#reset-cancel').click();
 ...
 await control(page,'#language','selectOption','ja');await expect(page.locator('#undo')).toHaveText('一手戻す');expect(errors).toEqual([]);
});
