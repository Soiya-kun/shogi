...
全兵種に槍突き・斬撃・一斉射撃・符術・軍配などの戦闘演出があります。AIは標準で約3秒に1手、戦闘部分も約3秒です。接近・進駐を含む約4.2秒の演出中も次のAIが考えて指し、別部隊が並行して動きます。「AIのテンポ」で約1秒・3秒・5秒を選び、「戦闘演出」「効果音」で表示と音を切り替えられます。速い設定ではAIの探索を軽くします。[戦闘演出の実装と確認結果](docs/combat-implementation.md)を参照してください。

飛車は8騎の騎馬武者隊です。成ると翼のない東洋の龍へ乗り替わり、空中を移動します。捕獲して持ち駒から打つと馬の部隊へ戻ります。

「昇龍」「援軍」「王手」「詰み→凱歌」とAI指揮への返答を漢字・日英字幕で表示します。戦法成立、解析で確認した好手、同じ部隊の帰参や戦功にも対応。「戦局演出の設定」で表示・字幕・音声・解析を切り替えられます。大将の声は未収録で、後から素材を追加できます。[100案の実装状況と検証結果](docs/battle-presentation-status.md)を参照してください。

視点は左ドラッグで回転、右ドラッグで平行移動、ホイールでカーソル位置を中心に拡大縮小できます。「全景」で盤面中央へ戻ります。部隊の選択・移動は左クリック、スマホでは1本指で回転、2本指で拡大縮小・平行移動します。矢印キーとEnterでも部隊を選択・移動できます。
...
- `dist/combat-motion.mjs`、`dist/combat-effects.js`：兵種別の戦闘動作、粒子の一括描画、効果音
