...
| コマンド | 結果・範囲 |
| --- | --- |
| `npm test` | 27件成功。既存ルール・保存・地形・隊形に加え、AIのUSI/SFEN・全履歴、依頼の取消、連続指示、先後の担当、非同期演出、復元・戻す・投了、定跡・候補比較、期限超過、成りによる馬と龍、戦闘姿勢と転倒後の進駐を検証 |
| `npm run build` | GLB構造、全11原型とLOD、馬・東洋の龍の部位とマス内への収まり、地面の高さ、編集元、同梱9枚のJPEG、CC0出典、容量上限、AI配布物の容量とSHA-256・WASM構造・対応ソースを検証 |
| `npm run test:web` | 25件成功。既存9件、実物WASMのAI対局・停止・指示・保存・異常復旧・詰み・スマホ操作・静的配信6件、演出中の取り返し・即時投入・巻き戻し等1件、右ドラッグと左右クリックの分離3件、全兵種の戦闘・並行再生・表示世代・演出OFFの一致・負荷上限・スマホ・標準3秒間隔6件を検証 |
