...
- 地上兵と騎馬武者の接地、成り飛車の龍の浮遊、地面に沿う細い区切り線、移動先表示、隊列移動・再整列・捕獲・再投入・昇格。
- 全兵種の攻撃・命中・転倒・進駐、矢・札・術式・炎・土煙と効果音。戦闘の完了を待たずに次の着手・AI思考を進め、別部隊を並行再生する。
- 全景と選択部隊への接近、左ドラッグで平行移動、右ドラッグで回転、俯瞰、駒名表示。札・旗とマスから部隊を選択。
...
- 漢字・日英字幕・音声差し込み・演出設定と、局面／解析／部隊戦歴に基づく44案の戦局演出。古い演出の取消、優先順位・重複排除、帰参・雪辱などの部隊ID追跡。[100案の実装状況と検証結果](battle-presentation-status.md)で対応範囲を管理する。音声は未収録。
...
| コマンド | 結果・範囲 |
| --- | --- |
| `npm test` | 47件成功。戦局演出の20テスト群（局面・解析証跡・先後反転・部隊ID・指揮返答・取消・復元）と、既存ルール・保存・地形・隊形に加え、AIのUSI/SFEN・全履歴、依頼の取消、連続指示、先後の担当、非同期演出、復元・戻す・投了、定跡・候補比較、期限超過、成りによる馬と龍、戦闘姿勢と転倒後の進駐を検証 |
| `npm run build` | GLB構造、全11原型とLOD、馬・東洋の龍の部位とマス内への収まり、地面の高さ、編集元、同梱9枚のJPEG、CC0出典、容量上限、AI配布物の容量とSHA-256・WASM構造・対応ソースを検証 |
| `npm run test:web` | 32件成功。戦局演出7件（基本演出・終局連続表示・AI返答・スマホ・実解析・HTMLAudio再生と取消）に加え、既存9件、実物WASMのAI対局・停止・指示・保存・異常復旧・詰み・スマホ操作・静的配信6件、演出中の取り返し・即時投入・巻き戻し等1件、右ドラッグと左右クリックの分離3件、全兵種の戦闘・並行再生・表示世代・演出OFFの一致・負荷上限・スマホ・標準3秒間隔6件を検証 |
| Blender再生成 | 地形・兵士原型・LOD・GLB・編集元の書き出し成功 |
| Blender再書き出し | 保存済みの編集元から、原型を保持しLODを更新してGLBを書き出し成功 |

ブラウザではクリック、キー操作、タッチ、一手戻す、保存復元、捕獲、持ち駒投入、成りの選択と復元、視点・札切替、全景・接近のLOD変更、接近中の旗選択と移動、横はみ出し、GLB読込失敗時の再試行表示を確認。JavaScript例外とシェーダーを含むコンソールエラーがないこと、初期配置392人の地上兵・馬が地面に沿い、飛車の成り後だけ龍が浮遊することも検証した。

飛車の追加検証では、8騎の馬、先後の成りによる東洋の龍への切替、空中の龍からの選択、浮遊、捕獲後の馬としての再投入、保存復元、一手戻す、動きを減らす設定、スマホの6騎表示を確認した。原型・遠景モデルの両方で馬の四肢・尾と龍の長い胴・枝角・ひげが揃い、装備を含めて各マスに収まることも検証する。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| PC全景 | 392 | 0 | 226 | 593,447 | 13.23ms | 13.39ms |
| PC接近 | 392 | 24 | 194 | 917,706 | 13.20ms | 13.38ms |
| スマホ幅・全景 | 240 | 0 | 230 | 503,119 | 13.19ms | 13.39ms |
...
地形GLBは21,379,948 bytes、兵士GLBは3,979,768 bytes、テクスチャ9枚は7,057,732 bytes。初回アート素材は合計約30.92MiB。動作時はこれらのローカルファイルを使い、BlenderやMCPへの接続は不要。
...
![馬に乗った武者8騎](verification/rook-cavalry-close.png)

成ると東洋の龍へ乗り替わる：

![翼のない東洋の龍に乗る武者8騎](verification/rook-eastern-dragon-close.png)

[騎馬隊の描画計測](verification/rook-cavalry-performance.json)、[龍の描画計測](verification/rook-eastern-dragon-performance.json)。
...
## 2026-09-06：飛車を馬、成り後を東洋の龍へ変更

通常の飛車を馬と和甲冑の武者に変更し、8騎の3・2・3列の配置を維持した。馬具、手綱、鞍、鐙、たてがみ、尾、独立した四肢を備え、地上を歩く。成った飛車は専用原型の東洋の龍へ切り替え、翼を使わずに浮遊する。長い胴と曲がった尾、鱗と腹板、枝分かれした角、ひげ、たてがみ、短い四肢と爪を備える。

描画原型は通常の`R`と成り後専用の`D`に分け、将棋の駒種と保存形式は`R`・成りフラグのまま維持した。成りの移動演出が終わると乗り替わり、同じ部隊の次の移動が先に来た場合は旧演出を短縮して切り替える。捕獲・持ち駒からの投入・戻す・復元でも、実際の成り状態から乗り物を選び直す。

Blenderでは2原型とそれぞれのLOD、編集元のサンプルを更新した。他の全原型・LOD・地形のメッシュハッシュと、地形GLBが不変であることを確認。テスト26件・ブラウザ16件・アセット検証が成功し、先後の成り、捕獲した龍の馬としての再投入、保存復元、戻す、浮遊と省略表示、空中のカーソルズーム、AIの連続着手を維持した。PC接近とスマホ全景も既存の描画上限を通過した。

上記に約29mからの近距離画面を掲載。モデルの編集方法は [Blender編集手順](../assets/blender/README.md#飛車の騎馬武者と成り後の龍) を参照。
...
## 2026-09-06：右ドラッグで視点を平行移動

マウス操作を左ドラッグの回転と右ドラッグの平行移動に分けた。右ボタンを押した位置の地面・兵士を基準に、カメラと注視点を一緒に移動する。つかんだ点がカーソルに追従し、向きと拡大率を維持する。カメラが地面に入る移動は手前で止める。

右クリックでは部隊を選択・移動せず、戦場内のコンテキストメニューを抑制する。右ドラッグ中はつかむカーソルを表示し、画面外でボタンを離した場合、ポインター捕捉が失われた場合、ウィンドウが非アクティブになった場合に操作を解除する。視点の補間中もその時点から操作を引き継ぎ、ホイールは引き続きカーソル位置を中心に拡大縮小する。

追加のブラウザテスト3件で、右ドラッグ前後の向き・拡大率と基準点の追従（1 CSSピクセル未満）、左回転後・拡大後の平行移動、右クリックでの誤操作防止、左クリックでの移動、画面外での解除、ポインター捕捉の喪失、補間中の割込み、地面への侵入防止を確認した。

`npm test`の26件、`npm run build`のアセット検証、`npm run test:web`の全19件が成功した。既存のカーソルズーム・スマホのピンチ・対局操作・AIも維持する。

[回転・拡大・右ドラッグ後の画面](verification/right-drag-pan.png)。

## 2026-09-06：全兵種の非同期戦闘

[駒取り・部隊戦闘の設計](capture-animation-design.md)に基づいて、足軽・長槍・弓足軽・重装武者・旗本・陰陽師・騎馬武者・本陣と成り飛車の龍へ、攻撃・応戦・命中・転倒・進駐を実装した。追加の希望を反映し、標準のAIは約3秒に1手、戦闘は約3秒、接近・進駐を含む全体は約4.2秒とした。AIと人間の着手は演出完了を待たず、別部隊の戦闘を継続する。再移動・取り返しだけを中断し、捕獲直後の打ちは旧残像と別の表示世代を使う。戻す・リセット・復元・演出OFFでは残像・粒子・音を解除する。

実物WASMの速い設定で12手を進め、最大3戦闘・4部隊が動き、過去の戦闘中に新しい局面から思考していることを確認した。同じ棋譜を演出OFFで再生した結果も一致する。標準設定の間隔は約3.003秒で、その時点で前の戦闘も続いていた。兵種・成り・陣営11例の接近表示、スマホ幅の同時動作上限とタッチ、動きを減らす設定も確認した。全25件のブラウザテスト、27件の単体テスト、アセット検証が成功した。

詳しい構成、上限、操作と画面は[戦闘演出の実装記録](combat-implementation.md)。

## 2026-09-06：ドラッグ操作の左右入れ替え

左ドラッグを平行移動、右ドラッグを視点回転に変更し、画面内の操作案内も更新。左クリックの部隊選択・移動、右クリックの誤操作防止、タッチ操作は維持。ブラウザのカメラ操作・ズーム6件とアセット検証が成功。

## 2026-09-06：対局リセットの専用モーダル

window.confirmを廃止し、対局画面に合わせた確認モーダルを実装。キャンセル・Escで閉じ、確認ボタンでAI停止と初期配置へのリセットを行う。背景操作はモーダル中に遮断する。ブラウザ3件で取消・繰り返し実行・保存復元・戦闘中のリセット・古い戦局演出の取消を確認し、画面も目視確認。アセット検証成功。

## 2026-09-06：地面に水平なドラッグ移動

左ドラッグの基準面をカメラに平行な面から、つかんだ位置を通る水平なXZ面へ変更。視点の高さ・向き・拡大率を保って地面に沿う前後左右の移動を行う。丘への侵入を防ぐ移動制限は維持。ブラウザ3件で高さ不変、回転・拡大後の追従、クリック分離、捕捉解除、地面への侵入防止を確認。アセット検証成功。
